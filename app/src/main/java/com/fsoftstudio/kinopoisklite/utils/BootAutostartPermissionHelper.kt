...
class BootAutostartPermissionHelper private constructor() {
    ...
    /**
     * Checks the autostart permission is enabled (work for not all smartphone models).
     *
     * @param context
     */
    fun isAutoStartEnabled(context: Context): Boolean {

        val APP_AUTO_START_PERMISSION_METHOD_NAME = "checkOpNoThrow"
        val APP_AUTO_START_PERMISSION_CODE = 10008

        try {
            val appOpsManager =
                context.getSystemService(Context.APP_OPS_SERVICE) as AppOpsManager
            val method: Method = AppOpsManager::class.java.getMethod(
                APP_AUTO_START_PERMISSION_METHOD_NAME,
            ...
            val result = method.invoke(
                appOpsManager,
                APP_AUTO_START_PERMISSION_CODE,
                Process.myUid(),
                context.packageName
            ) as Int
    ...
    companion object {
        ...
        private val myInstance by lazy { BootAutostartPermissionHelper() }

        fun getInstance(): BootAutostartPermissionHelper {
            return myInstance
        }
    }
